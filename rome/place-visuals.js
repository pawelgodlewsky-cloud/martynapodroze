...
export function hasRealVisual(itemOrId) {
  return photos.has(typeof itemOrId === "string" ? itemOrId : itemOrId?.id);
}
export function placeVisual(item) {
  if(hasRealVisual(item)) return `<img class="point-image" src="assets/places/${item.id}.jpg" alt="${esc(item.id==='campidoglio'?'Wilczyca Kapitolińska przy Piazza del Campidoglio':item.id==='vatican-museums'?'Galeria Map w Muzeach Watykańskich':item.name)}" loading="lazy" decoding="async"><small class="visual-caption">Zdjęcie z podróży</small>`;
  return `<svg class="point-image" viewBox="0 0 600 800" role="img" aria-label="${esc(item.name)} — ilustracja symboliczna"><rect width="600" height="800" fill="#e9eddf"/><circle cx="460" cy="185" r="28" fill="#e4ee91"/><g transform="translate(0 250)" fill="#d2dbbd" stroke="#36564a" stroke-width="5" stroke-linejoin="round" stroke-linecap="round">${motifs[types[item.id]]||motifs.view}</g></svg><small class="visual-caption">${esc(item.name)} · ilustracja symboliczna</small>`;
}
